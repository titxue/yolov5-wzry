...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if frame_counter % 15 == 0:  # 每30帧保存一次
            # if True:
                # 根据图像计数器决定是保存到训练子集还是验证子集
                if image_counter % 10 == 9:  # 每10张图片中的第9张
                    save_dir = validation_dir  # 保存到验证子集
                    label_dir = label_validation_dir  # 验证标签保存路径
                else:  # 其他图片
                    save_dir = train_dir  # 保存到训练子集
                    label_dir = label_train_dir  # 训练标签保存路径

                p = Path(p)  # to Path
                # 生成唯一的文件名，包括原始视频的文件名和帧编号
                save_path = str(
                    save_dir / f"{p.stem}_frame_{frame_counter}.jpg")  # im_frame_30.jpg, im_frame_60.jpg, etc.
                txt_path = str(label_dir / f"{p.stem}_frame_{frame_counter}") # im_frame_30.txt, im_frame_60.txt, etc.
                ...
                if len(det):
                    ...
                    for c in det[:, 5].unique():
                     ...
                     # Add a flag to track whether the current frame has been saved
                    frame_saved = False

                    # Write results
                    for *xyxy, conf, cls in reversed(det):
                        if conf >= confidence_threshold:  # 只保存置信度高于阈值的结果
                            if save_txt:  # Write to file
                                xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(
                                    -1).tolist()  # normalized xywh
                                line = (cls, *xywh, conf) if save_conf else (cls, *xywh)  # label format
                                with open(f'{txt_path}.txt', 'a') as f:  # 注意这里我们改用'w'模式打开文件，以覆盖之前的内容
                            ...
                            #             c] / f'{p.stem}_frame_{frame_counter}.jpg', BGR=True)

                            if not frame_saved:  # 如果当前帧还未被保存
